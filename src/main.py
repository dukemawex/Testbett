...
def check_secrets(settings) -> bool:
    ...
    if not settings.SPORTSBOOK_BASE_URL:
        missing.append("SPORTSBOOK_BASE_URL")
    if missing:
...
def main() -> None:
    ...
    settings = Settings()
    logger.info("DRY_RUN=%s  LOG_LEVEL=%s  LLM=%s",
                settings.DRY_RUN, settings.LOG_LEVEL,
                "enabled" if settings.OPENAI_API_KEY else "stub")
    ...
    stats_client = get_stats_client(settings.FOOTBALL_DATA_API_KEY, settings.COMPETITION, settings.SEASON)
    ...
    # 6. LLM analyst (optional – stub when no OPENAI_API_KEY)
    from src.llm.llm_client import get_llm_client
    from src.llm.bet_analyst import get_bet_analyst

    llm_client = get_llm_client(settings.OPENAI_API_KEY, settings.OPENAI_MODEL)
    llm_analyst = get_bet_analyst(llm_client)
    logger.info("LLM analyst ready (model=%s)", getattr(llm_client, 'model', 'stub'))
    ...
    sportsbook = get_sportsbook(
        dry_run=settings.DRY_RUN,
        api_key=settings.SPORTSBOOK_KEY,
        base_url=settings.SPORTSBOOK_BASE_URL,
    )
    bankroll = BankrollManager(settings.BANKROLL_FILE, settings.INITIAL_BANKROLL)
    executor = Executor(sportsbook, bankroll, settings, llm_analyst=llm_analyst)
